...
import { getAppConfig, getBoardMembers } from './services/dataService';
...
const App: React.FC = () => {
  ...
  // ✅ Dùng Supabase thật để lấy dữ liệu sessions
  useEffect(() => {
    async function fetchSessionsFromSupabase() {
      try {
        ...
        // Tính thống kê theo dữ liệu thật
        ...
        console.error("⚠️ Lỗi kết nối Supabase:", err);
      }
    }
      ...
      <aside className="hidden md:flex flex-col w-72 bg-white border-r border-slate-200 h-screen fixed top-0 left-0 z-30 shadow-[4px_0_24px_rgba(0,0,0,0.02)]">
        ...
        <nav className="flex-1 px-4 space-y-1 mt-4">
          ...
          {navItems.map((item) => (
            ...
            >
              <span
                className={`mr-3 transition-colors ${view === item.id ? 'text-orange-600' : 'text-slate-400 group-hover:text-slate-600'}`}
              >
                {item.icon}
              </span>
              {item.label}
